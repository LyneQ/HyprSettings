...
export default function HyprGroupsChild({
  ...
  // Keep internal state in sync with prop updates (including falsy values like 0/''/false)
  useEffect(() => {
    setValue(variable)
  }, [variable])
    ...
    case 'number': {
      ...
      return (
          <div className="hypr-groups-child__number-container">
              <button className={className ?? '' + "hypr-groups-child__number hypr-groups-child-input"} onClick={() => handleChange(numVal - 1)}>-</button>
              <button className={className ?? '' + "hypr-groups-child__number hypr-groups-child-input"} onClick={() => handleChange(numVal + 1)}>+</button>
              <input
                  type="number"
