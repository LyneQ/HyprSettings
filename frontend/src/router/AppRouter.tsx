...
function Guarded({guards, deniedFallback, element}: { guards?: GuardFn[], deniedFallback?: React.ReactNode, element: React.ReactNode }) {
  const location = useLocation()
...
function buildRouteElements(defs: RouteDefinition[], deniedFallback?: React.ReactNode, loadingFallback?: React.ReactNode): React.ReactNode {
  return defs.map((def, idx) => {
    ...
    if (def.redirectTo !== undefined) {
      ...
      element = <Suspense fallback={loadingFallback ?? <div>Loading…</div>}>
        <LazyComp />
      </Suspense>
    }
